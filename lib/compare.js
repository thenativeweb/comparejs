...
var isSubsetStructure = function (derived, base) {
  for (var i in derived) {
    if (typeof base[i] !== typeof derived[i] || (typeof derived[i] === 'object' && !isSubsetStructure(derived[i], base[i]))) {
      return false;
    }
  }
  return true;
...
for (var i in cmp) {
  if (cmp.hasOwnProperty(i)) {
    if (i.length === 3) {
      module.exports[i] = module.exports[alias[i]] = setup(cmp[i], processTypesStructure);
    } else {
      module.exports[i] = module.exports[alias[i]] = setup(cmp[i], processTypes);
    }
  }
}
