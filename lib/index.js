(function (exports) {
  ...
  var cmp = {
    ...
    eqs: function (first, second) {
      // eqs only works for objects.
      if(typeof first !== 'object' || typeof second !== 'object') {
        return false;
      }

      // If at least one object is null, they do not need to be compared by structure.
      if(first === null || second === null) {
        // If both objects are null, their structure is equal; otherwise not.
        return first === null && second === null;
      }

      // If at least one object is an array, they are not equal by structure.
  ...
  for(var i in cmp) {
    if(cmp.hasOwnProperty(i)) {
      exports[i] = exports[alias[i]] = setup(cmp[i]);
    }
  }
